...
st.markdown('<div style="text-align:center; padding:20px; border-bottom:5px solid #4db6ac;"><h2>M.A.M. COLLEGE OF ENGINEERING</h2></div>', unsafe_allow_html=True)
...
def get_labels():
    # Try rearranging this list if the results are consistently swapped
    ...
    if file:
        ...
        if st.button("RUN AI"):
            ...
            if model:
                ...
                # DEBUG: This shows the raw scores for all 12 categories
                st.session_state['raw_scores'] = preds[0]
...
with c2:
    ...
    if 'res_label' in st.session_state:
        ...
        with st.expander("See Raw Probability Scores"):
            st.write(st.session_state['raw_scores'])
    else:
